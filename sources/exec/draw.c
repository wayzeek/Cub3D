...
void	draw_vert_ray(t_data *data, t_ray *ray)
{
	...
	id = ray->id;
	ray->sq_length = (1.0 / ray->perp_length * WIN_HEIGHT) * 300; //arbitrary
	if (ray->sq_length > WIN_HEIGHT || ray->sq_length < 0)
		ray->sq_length = WIN_HEIGHT;
//    if (id >= 0 && id <= 20)
//        printf("ray %d, lenght %d\n", id, ray->sq_length);
	if (ray->side_hit == 0)
		draw_seg(data, (t_vector){id, WIN_HEIGHT / 2 - ray->sq_length / 2}, \
	(t_vector){id, WIN_HEIGHT / 2 + ray->sq_length / 2}, N_WALL);
	else if (ray->side_hit == 1)
		draw_seg(data, (t_vector){id, WIN_HEIGHT / 2 - ray->sq_length / 2}, \
	(t_vector){id, WIN_HEIGHT / 2 + ray->sq_length / 2}, S_WALL);
	else if (ray->side_hit == 2)
		draw_seg(data, (t_vector){id, WIN_HEIGHT / 2 - ray->sq_length / 2}, \
	(t_vector){id, WIN_HEIGHT / 2 + ray->sq_length / 2}, E_WALL);
	else if (ray->side_hit == 3)
		draw_seg(data, (t_vector){id, WIN_HEIGHT / 2 - ray->sq_length / 2}, \
	(t_vector){id, WIN_HEIGHT / 2 + ray->sq_length / 2}, W_WALL);
}
